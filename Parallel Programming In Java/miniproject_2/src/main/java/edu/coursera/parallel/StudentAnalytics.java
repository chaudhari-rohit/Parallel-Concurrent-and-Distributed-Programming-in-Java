...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(final Student[] studentArray) {
        final ConcurrentMap<String, Long> map =
                    Arrays.stream(studentArray).parallel()
                          ...
                          .collect(Collectors.groupingByConcurrent(name -> name.getFirstName(), Collectors.counting()));
        final String mostCommonName = map.entrySet().stream().parallel()
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        return (int) Arrays.stream(studentArray).parallel()
                           ...
                           .filter(student -> student.getGrade() < 65).count();
    }
}
